...
DISTRIBUTIONS = {
    "gaussian": {
        ...
        "class": lambda theta: tfp.distributions.Normal(
            loc=theta["mu"], scale=tf.exp(theta["log_sigma"])
        ),
    },
    "softplus gaussian": {
        "parameters": {
    ...
    "gaussian mixture": {
        ...
        "class": lambda theta: tfp.distributions.Mixture(
            ...
            components=[
                tfp.distributions.MultivariateNormalDiag(loc=m, scale_diag=tf.exp(s))
    ...
    "log-normal": {
        "parameters": {
            "mean": {"support": [-np.inf, np.inf], "activation function": tf.identity},
            "variance": {"support": [0, np.inf], "activation function": tf.nn.softplus},
        },
    ...
    "exponentially_modified_gaussian": {
        "parameters": {
            ...
            "scale": {"support": [0, np.inf], "activation function": tf.nn.softplus},
            "rate": {"support": [0, np.inf], "activation function": tf.nn.softplus},
        },
    ...
    "categorical": {
        "parameters": {
            "logits": {"support": [-np.inf, np.inf], "activation function": tf.identity}
        },
        "class": lambda theta: tfp.distributions.Categorical(logits=theta["logits"]),
    },
    "bernoulli": {
        "parameters": {
            "logits": {"support": [-np.inf, np.inf], "activation function": tf.identity}
        },
        "class": lambda theta: tfp.distributions.Bernoulli(logits=theta["logits"]),
    },
    "poisson": {
        "parameters": {
            "log_lambda": {"support": [-10, 10], "activation function": tf.identity}
        },
    ...
    "constrained poisson": {
        "parameters": {
            "lambda": {"support": [0, 1], "activation function": tf.nn.softmax}
        },
        "class": lambda theta, N: tfp.distributions.Poisson(rate=theta["lambda"] * N),
    },
    "lomax": {
        "parameters": {
            "log_concentration": {
            ...
            "log_scale": {"support": [-10, 10], "activation function": tf.identity},
        },
    ...
    "zero-inflated poisson": {
        ...
        "class": lambda theta: ZeroInflated(
            dist=tfp.distributions.Poisson(rate=tf.exp(theta["log_lambda"])),
            pi=theta["pi"],
        ),
    },
    "negative binomial": {
        "parameters": {
            "p": {"support": [0, 1], "activation function": tf.sigmoid},
            "log_r": {"support": [-10, 10], "activation function": tf.identity},
        },
...
LATENT_DISTRIBUTIONS = {
    "gaussian": {
        "prior": {"name": "gaussian", "parameters": {"mu": 0.0, "log_sigma": 0.0}},
        "posterior": {"name": "gaussian", "parameters": {}},
    },
    "unit-variance gaussian": {
        "prior": {"name": "gaussian", "parameters": {"mu": 0.0, "log_sigma": 0.0}},
        "posterior": {"name": "gaussian", "parameters": {"log_sigma": 0.0}},
    },
}
