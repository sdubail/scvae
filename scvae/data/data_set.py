...
class DataSet:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.name, data_set_dictionary = parsing.parse_input(input_file_or_name)

        # Directories and paths for data set
        if directory is None:
            directory = defaults["data"]["directory"]
        self._directory = os.path.join(directory, self.name)
        self._preprocess_directory = os.path.join(self._directory, PREPROCESS_SUFFIX)
        self._original_directory = os.path.join(self._directory, ORIGINAL_SUFFIX)
        ...
        if data_set_dictionary:
            ...
            parsing.save_data_set_dictionary_as_json_file(
                data_set_dictionary, self.name, self._directory
            )
        ...
        if data_format == "infer":
            ...
            if data_format_from_metadata and data_format_from_metadata != data_format:
                raise ValueError(
                    "Data format already specified in metadata "
                    "and cannot be changed (is `{}`; wanted `{}`).".format(
                        data_format_from_metadata, data_format
                    )
                )
        self.data_format = data_format

        # Terms for data set
        self.terms = _postprocess_terms(self.specifications.get("terms", DEFAULT_TERMS))
        ...
        self.feature_dimensions = self.specifications.get("feature dimensions")
        ...
        explained_standard_deviations = kwargs.get("explained_standard_deviations")
        ...
        self.superset_label_sorter = _create_label_sorter(sorted_superset_class_names)
        ...
        if self.noisy_preprocessing_methods:
            self.noisy_preprocess = processing.build_preprocessor(
                self.noisy_preprocessing_methods, noisy=True
            )
        else:
        ...
        if self.feature_selection_method:
            ...
            if feature_selection == "keep_variances_above":
                ...
                if self.number_of_features is not None:
                    feature_selection_parameters = [int(self.number_of_features / 2)]
    ...
    def update(
        ...
        if values is not None:
            ...
            if example_names is not None:
                ...
                if self.example_names.ndim > 1:
                    raise ValueError(
                        "The list of example names is multi-dimensional: {}.".format(
                            self.example_names.shape
                        )
                    )
                ...
                if n_examples_from_values != n_examples:
                    raise ValueError(
                        ...
                        "is not the same as the number of example names ({}).".format(
                            n_examples_from_values, n_examples
                        )
                    )
            ...
            if feature_names is not None:
                ...
                if self.feature_names.ndim > 1:
                    raise ValueError(
                        "The list of feature names is multi-dimensional: {}.".format(
                ...
                if n_featues_from_values != n_features:
                    raise ValueError(
                        ...
                        "is not the same as the number of feature names ({}).".format(
                            n_featues_from_values, n_features
                        )
                    )
        ...
        if labels is not None:
            if issubclass(labels.dtype.type, float):
            ...
            if self.label_superset:
                ...
                self.superset_class_names = numpy.unique(self.superset_labels).tolist()
    ...
    def update_predictions(
        ...
        if predicted_superset_labels is not None:
            ...
            if predicted_superset_class_names is not None:
                self.predicted_superset_class_names = predicted_superset_class_names
            else:
                self.predicted_superset_class_names = numpy.unique(
                    self.predicted_superset_labels
                ).tolist()
            ...
            if set(self.predicted_superset_class_names) < set(
                ...
                self.predicted_superset_class_palette = self.superset_class_palette
                self.predicted_superset_label_sorter = self.superset_label_sorter
    ...
    def load(self):
        ...
        if os.path.isfile(sparse_path):
            ...
            data_dictionary = internal_io.load_data_dictionary(path=sparse_path)
            print()
        else:
            urls = self.specifications.get("URLs", None)
            original_paths = loading.acquire_data_set(
                title=self.title, urls=urls, directory=self._original_directory
            )

            loading_time_start = time()
            data_dictionary = loading.load_original_data_set(
                paths=original_paths, data_format=self.data_format
            )
            ...
            if loading_duration > MINIMUM_NUMBER_OF_SECONDS_BEFORE_SAVING:
                ...
                internal_io.save_data_dictionary(
                    data_dictionary=data_dictionary, path=sparse_path
                )

                print()

        data_dictionary["values"] = sparse.SparseRowMatrix(data_dictionary["values"])
    ...
    def preprocess(self):
        if (
            not self.map_features
            and not self.preprocessing_methods
            and not self.feature_selection
        ...
        if os.path.isfile(sparse_path):
            ...
            if self.map_features and not self.features_mapped:
                ...
                print("Features mapped ({}).".format(format_duration(duration)))
            ...
            if not self.preprocessed and self.preprocessing_methods:
                ...
                duration = time() - start_time
                print("Values preprocessed ({}).".format(format_duration(duration)))
            ...
            if self.feature_selection:
                values_dictionary, feature_names = processing.select_features(
                    {"original": values, "preprocessed": preprocessed_values},
                    self.feature_names,
            ...
            if preprocessing_duration > MINIMUM_NUMBER_OF_SECONDS_BEFORE_SAVING:
                if not os.path.exists(self._preprocess_directory):
                    os.makedirs(self._preprocess_directory)
    ...
    def binarise(self):
        ...
        if os.path.isfile(sparse_path):
            ...
            if self.preprocessing_methods != binarise_preprocessing:
                ...
                duration = time() - start_time
                print("Values binarised ({}).".format(format_duration(duration)))
    ...
    def split(self, method=None, fraction=None):
        ...
        if os.path.isfile(sparse_path):
            ...
            split_data_dictionary = internal_io.load_data_dictionary(path=sparse_path)
            if self.map_features:
                self.features_mapped = True
                self.terms = _update_tag_for_mapped_features(self.terms)
            print()
            ...
            if splitting_duration > MINIMUM_NUMBER_OF_SECONDS_BEFORE_SAVING:
                ...
                internal_io.save_data_dictionary(
                    data_dictionary=split_data_dictionary, parse=sparse_path
                )
                print()
        ...
        training_set = DataSet(
            ...
            batch_indices=(split_data_dictionary["training set"]["batch indices"]),
        ...
        test_set = DataSet(
            ...
            binarised_values=(split_data_dictionary["test set"]["binarised values"]),
        ...
        print(
            "Data sets with {} features{}{}:\n".format(
                ...
                (
                    ...
                    if self.number_of_superset_classes
                    else ""
                ),
            )
            + "    Training set: {} examples.\n".format(training_set.number_of_examples)
            + "    Validation set: {} examples.\n".format(
                validation_set.number_of_examples
            )
            + "    Test set: {} examples.".format(test_set.number_of_examples)
        )
    ...
    def _build_preprocessed_path(
        ...
        if feature_selection_method:
            ...
            if feature_selection_parameters:
                for parameter in feature_selection_parameters:
                    feature_selection_part += "_" + normalise_string(str(parameter))
        ...
        if example_filter_method:
            ...
            if example_filter_parameters:
                for parameter in example_filter_parameters:
                    example_filter_part += "_" + normalise_string(str(parameter))
        ...
        if splitting_method:
            ...
            if (
                splitting_method == "indices"
                and len(split_indices) == 3
...
def _create_class_palette(class_names):
    ...
    if len(class_names) <= len(brewer_palette):
        class_palette = {c: brewer_palette[i] for i, c in enumerate(class_names)}
    else:
        class_palette = None
...
def _create_superset_class_palette(class_palette, label_superset):
    if class_palette is None or label_superset is None or label_superset == "infer":
        ...
        for superset_label, labels_in_superset_label in label_superset.items():
            ...
            for label_in_superset_label in labels_in_superset_label:
                superset_label_colours.append(class_palette[label_in_superset_label])
