...
from scvae.models.utilities import (
    ...
    build_data_string,
    build_training_string,
    check_run_id,
    clear_log_directory,
    copy_model_directory,
    correct_model_checkpoint_path,
    dense_layer,
    dense_layers,
...
class GaussianMixtureVariationalAutoencoder:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        reconstruction_distribution = parse_distribution(reconstruction_distribution)
        ...
        if latent_distribution is None:
            latent_distribution = defaults["models"]["latent_distribution"][self.type]
        ...
        if prior_probabilities_method in ["uniform", "learn"]:
            ...
            raise NotImplementedError(
                "`{}` method for setting prior probabilities not implemented.".format(
                    prior_probabilities_method
                )
            )
        ...
        number_of_monte_carlo_samples = kwargs.get("number_of_monte_carlo_samples")
        if number_of_monte_carlo_samples is None:
            number_of_monte_carlo_samples = defaults["models"]["number_of_samples"]
        else:
        ...
        number_of_importance_samples = kwargs.get("number_of_importance_samples")
        if number_of_importance_samples is None:
            number_of_importance_samples = defaults["models"]["number_of_samples"]
        else:
        ...
        if minibatch_normalisation is None:
            minibatch_normalisation = defaults["models"]["minibatch_normalisation"]
        ...
        if self.batch_correction:
            if number_of_batches is None:
                raise TypeError(
                    "The number of batches for batch correction was not " "provided."
                )
        ...
        if number_of_warm_up_epochs is None:
            number_of_warm_up_epochs = defaults["models"]["number_of_warm_up_epochs"]
        ...
        with self.graph.as_default():
            self.x = tf.placeholder(
                dtype=tf.float32, shape=[None, self.feature_size], name="X"
            )
            self.t = tf.placeholder(
                dtype=tf.float32, shape=[None, self.feature_size], name="T"
            )
            ...
            self.kl_weight = tf.constant(
                self.kl_weight_value, dtype=tf.float32, name="kl_weight"
            )

            self.warm_up_weight = tf.placeholder(
                dtype=tf.float32, shape=[], name="warm_up_weight"
            )
            parameter_summary = tf.summary.scalar(
                name="warm_up_weight", tensor=self.warm_up_weight
            )
            self.parameter_summary_list.append(parameter_summary)

            self.total_kl_weight = tf.multiply(
                self.warm_up_weight, self.kl_weight, name="total_kl_weight"
            )
            total_kl_weight_summary = tf.summary.scalar(
                name="total_kl_weight", tensor=self.total_kl_weight
            )
            ...
            self.n_iw_samples = tf.placeholder(
                dtype=tf.int32, shape=[], name="number_of_iw_samples"
            )
            self.n_mc_samples = tf.placeholder(
                dtype=tf.int32, shape=[], name="number_of_mc_samples"
            )

            if self.batch_correction:
                self.batch_indices = tf.placeholder(
                    dtype=tf.int32, shape=[None, 1], name="batch_indices"
                )

            if self.use_count_sum_as_feature:
                self.count_sum_feature = tf.placeholder(
                    dtype=tf.float32, shape=[None, 1], name="count_sum_feature"
                )
            if self.use_count_sum_as_parameter:
                self.count_sum_parameter = tf.placeholder(
                    dtype=tf.float32, shape=[None, 1], name="count_sum"
                )
            ...
            self.sample_size = tf.placeholder(
                dtype=tf.int32, shape=[], name="sample_size"
            )
    ...
    def name(self):
        ...
        reconstruction_parts = [normalise_string(self.reconstruction_distribution_name)]
        ...
        reconstruction_parts.append("h_" + "_".join(map(str, self.hidden_sizes)))
        ...
        if len(self.dropout_parts) > 0:
            reconstruction_parts.append("dropout_" + "_".join(self.dropout_parts))

        if self.kl_weight_value != 1:
            reconstruction_parts.append("klw_{}".format(self.kl_weight_value))

        if self.number_of_warm_up_epochs:
            reconstruction_parts.append("wu_{}".format(self.number_of_warm_up_epochs))

        if self.proportion_of_free_nats_for_y_kl_divergence:
            reconstruction_parts.append(
                "fn_{}".format(self.proportion_of_free_nats_for_y_kl_divergence)
            )
    ...
    def description(self):
        ...
        description_parts.append(
            "hidden sizes: {}".format(", ".join(map(str, self.hidden_sizes)))
        )
        ...
        if "mixture" in self.latent_distribution_name:
            description_parts.append("latent clusters: {}".format(self.n_clusters))
            description_parts.append(
                "prior probabilities: " + self.prior_probabilities_method
            )

        description_parts.append(
            "reconstruction distribution: " + self.reconstruction_distribution_name
        )
        if self.k_max > 0:
            description_parts.append(
        ...
        if self.kl_weight_value != 1:
            description_parts.append("KL weight: {}".format(self.kl_weight_value))

        if self.minibatch_normalisation:
            description_parts.append("using batch normalisation for minibatches")
        ...
        if self.number_of_warm_up_epochs:
            description_parts.append(
                "using linear warm-up weighting for the first {} epochs".format(
        ...
        if self.proportion_of_free_nats_for_y_kl_divergence:
            description_parts.append(
                "proportion of free nats for y KL divergence: {}".format(
                    self.proportion_of_free_nats_for_y_kl_divergence
                )
            )
        ...
        if len(self.dropout_parts) > 0:
            description_parts.append(
                "dropout keep probability: {}".format(", ".join(self.dropout_parts))
            )
        ...
        if self.early_stopping_rounds:
            description_parts.append(
                "early stopping: after {} epoch with no improvements".format(
    ...
    def parameters(self):
        ...
        for parameter in parameters:
            parameters_string_parts.append(
                "{:{}}  {}".format(parameter.name, width, parameter.get_shape())
            )
    ...
    def log_directory(
        self, base=None, run_id=None, early_stopping=False, best_model=False
    ):
        if not base:
        ...
        if run_id:
            ...
            log_directory = os.path.join(log_directory, "run_{}".format(run_id))
    ...
    def early_stopping_status(self, run_id=None):
        ...
        early_stopping_log_directory = self.log_directory(
            run_id=run_id, early_stopping=True
        )
        ...
        if os.path.exists(log_directory):
            ...
            if os.path.exists(early_stopping_log_directory):
                stopped_early, epochs_with_no_improvement = early_stopping_status(
                    validation_losses, self.early_stopping_rounds
                )
    ...
    def train(
        ...
        **kwargs,
        ...
        if run_id:
            ...
            run_id = generate_unique_run_id_for_model(model=self, timestamp=start_time)
        ...
        if old_checkpoint:
            epoch_start = int(
                os.path.basename(old_checkpoint.model_checkpoint_path).split("-")[-1]
            )
        else:
            epoch_start = 0
        ...
        if temporary_log_directory:
            log_directory = self.log_directory(
                base=temporary_log_directory, run_id=run_id
            )
            early_stopping_log_directory = self.log_directory(
                base=temporary_log_directory, run_id=run_id, early_stopping=True
            )
            best_model_log_directory = self.log_directory(
                base=temporary_log_directory, run_id=run_id, best_model=True
            )

            temporary_checkpoint = tf.train.get_checkpoint_state(log_directory)

            if temporary_checkpoint:
                temporary_epoch_start = int(
                    os.path.basename(temporary_checkpoint.model_checkpoint_path).split(
                        "-"
                    )[-1]
                )
            else:
            ...
            best_model_log_directory = self.log_directory(
                run_id=run_id, best_model=True
            )

        # Training message
        data_string = build_data_string(
            data_set=training_set,
            reconstruction_distribution_name=(self.reconstruction_distribution_name),
        )
        ...
        if (
            ...
            print(
                "Log directory copied ({}).".format(format_duration(copying_duration))
            )
        ...
        if training_set.has_labels:
            class_names_to_class_ids = numpy.vectorize(
                lambda class_name: training_set.class_name_to_class_id[class_name]
            )
        ...
        if training_set.has_superset_labels:
            superset_class_names_to_superset_class_ids = numpy.vectorize(
                lambda superset_class_name: training_set.superset_class_name_to_superset_class_id[
                    superset_class_name
                ]
            )

            training_superset_label_ids = superset_class_names_to_superset_class_ids(
        ...
        print("Data prepared ({}).".format(format_duration(preparing_data_duration)))
        print()
        ...
        with tf.Session(graph=self.graph) as session:
            ...
            if checkpoint:
                ...
                model_checkpoint_path = correct_model_checkpoint_path(
                    checkpoint.model_checkpoint_path, log_directory
                )
                ...
                if validation_set:
                    ...
                    lower_bound_valid_maximum = lower_bound_valid_learning_curve.max()

                    self.stopped_early, epochs_with_no_improvement = (
                        self.early_stopping_status(run_id=run_id)
                    )
                    lower_bound_valid_early_stopping = lower_bound_valid_learning_curve[
                        -1 - epochs_with_no_improvement
                    ]

                restoring_duration = time() - restoring_time_start
                print(
                    "Earlier model parameters restored ({}).".format(
                ...
                print(
                    "Model parameters initialised ({}).".format(
            ...
            for epoch in range(epoch_start, number_of_epochs):
                if noisy_preprocess:
                    ...
                    print(
                        "Values noisily preprocessed ({}).".format(
                ...
                for i in range(0, n_examples_train, minibatch_size):
                    ...
                    # Prepare minibatch
                    minibatch_indices = shuffled_indices[i : (i + minibatch_size)]
                    ...
                    feed_dict_batch = {
                        ...
                        self.n_iw_samples: self.number_of_importance_samples[
                            "training"
                        ],
                        self.n_mc_samples: self.number_of_monte_carlo_samples[
                            "training"
                        ],
                    }

                    if self.batch_correction:
                        feed_dict_batch[self.batch_indices] = batch_indices_train[
                            minibatch_indices
                        ]
                    ...
                    _, minibatch_loss = session.run(
                        [self.optimiser, self.lower_bound], feed_dict=feed_dict_batch
                    )
                ...
                print(
                    "Epoch {} ({}):".format(epoch + 1, format_duration(epoch_duration))
                )
                ...
                q_z_means = numpy.zeros(shape=(self.n_clusters, self.latent_size))
                q_z_variances = numpy.zeros(shape=(self.n_clusters, self.latent_size))

                p_y_probabilities = numpy.zeros(shape=self.n_clusters)
                p_z_means = numpy.zeros(shape=(self.n_clusters, self.latent_size))
                p_z_variances = numpy.zeros(shape=(self.n_clusters, self.latent_size))

                if "full-covariance" in self.latent_distribution_name:
                    q_z_covariances = numpy.zeros(
                        shape=(self.n_clusters, self.latent_size, self.latent_size)
                    )
                ...
                q_y_logits_train = numpy.zeros(
                    shape=(n_examples_train, self.n_clusters), dtype=numpy.float32
                )
                z_mean_train = numpy.zeros(
                    shape=(n_examples_train, self.latent_size), dtype=numpy.float32
                )
                ...
                for i in range(0, n_examples_train, minibatch_size):
                    subset = slice(i, min(i + minibatch_size, n_examples_train))
                ...
                if numpy.isnan(lower_bound_train):
                    raise ArithmeticError(
                        "Aborting. The ELBO for the training set became " "indefinite."
                    )

                learning_curves["training"]["lower_bound"].append(lower_bound_train)
                ...
                if training_set.has_labels:
                    predicted_training_label_ids = map_cluster_ids_to_label_ids(
                        training_label_ids, training_cluster_ids, excluded_class_ids
                    )
                ...
                summary.value.add(
                    tag="losses/lower_bound", simple_value=lower_bound_train
                )
                summary.value.add(
                ...
                summary.value.add(
                    tag="losses/kl_divergence_z", simple_value=kl_divergence_z_train
                )
                summary.value.add(
                    tag="losses/kl_divergence_y", simple_value=kl_divergence_y_train
                )
                summary.value.add(tag="accuracy", simple_value=accuracy_train)
                if accuracy_superset_train:
                    summary.value.add(
                        tag="superset_accuracy", simple_value=accuracy_superset_train
                    )
                ...
                if validation_set is None:
                    for k in range(self.n_clusters):
                        ...
                        for l in range(self.latent_size):
                            ...
                            summary.value.add(
                                tag="prior/cluster_{}/variance/dimension_{}".format(
                                    k, l
                                ),
                ...
                training_summary_writer.add_summary(summary, global_step=epoch + 1)
                training_summary_writer.flush()

                # Printing training evaluation
                evaluation_string = "    {} set ({}): ".format(
                    training_set.kind.capitalize(), format_duration(evaluating_duration)
                )
                evaluation_metrics = [
                ...
                if accuracy_display:
                    evaluation_metrics.append("Acc: {:.5g}".format(accuracy_display))
                ...
                if validation_set:
                    ...
                    q_y_logits_valid = numpy.zeros(
                        shape=(n_examples_valid, self.n_clusters), dtype=numpy.float32
                    )
                    z_mean_valid = numpy.zeros(
                        shape=(n_examples_valid, self.latent_size), dtype=numpy.float32
                    )

                    for i in range(0, n_examples_valid, minibatch_size):
                        subset = slice(i, min(i + minibatch_size, n_examples_valid))
                        ...
                        if self.batch_correction:
                            feed_dict_batch[self.batch_indices] = batch_indices_valid[
                                subset
                            ]
                    ...
                    reconstruction_error_valid /= n_examples_valid / minibatch_size
                    ...
                    if validation_set.has_labels:
                        predicted_validation_label_ids = map_cluster_ids_to_label_ids(
                    ...
                    summary.value.add(
                        tag="losses/lower_bound", simple_value=lower_bound_valid
                    )
                    summary.value.add(
                        tag="losses/reconstruction_error",
                    ...
                    summary.value.add(
                        tag="losses/kl_divergence_z", simple_value=kl_divergence_z_valid
                    )
                    summary.value.add(
                        tag="losses/kl_divergence_y", simple_value=kl_divergence_y_valid
                    )
                    summary.value.add(tag="accuracy", simple_value=accuracy_valid)
                    ...
                    if lower_bound_valid < lower_bound_valid_early_stopping:
                        if epochs_with_no_improvement == 0:
                            ...
                            if current_checkpoint:
                                copy_model_directory(
                                    current_checkpoint, early_stopping_log_directory
                                )
                            saving_duration = time() - saving_time_start
                            print(
                                "        "
                                "Previous model parameters saved ({}).".format(
                            ...
                            print(
                                ...
                                "for {} epochs.".format(epochs_with_no_improvement + 1),
                            )
                ...
                saving_time_start = time()
                self.saver.save(session, checkpoint_file, global_step=epoch + 1)
                saving_duration = time() - saving_time_start
                print(
                    "    Model parameters saved ({}).".format(
                        format_duration(saving_duration)
                    )
                )

                # Saving best model parameters yet
                if validation_set and lower_bound_valid > lower_bound_valid_maximum:
                    print(
                    ...
                    current_checkpoint = tf.train.get_checkpoint_state(log_directory)
                    if current_checkpoint:
                        copy_model_directory(
                            current_checkpoint, best_model_log_directory
                        )
                    remove_old_checkpoints(best_model_log_directory)
                    saving_duration = time() - saving_time_start
                    print(
                        "    Best model parameters saved ({}).".format(
                            format_duration(saving_duration)
                        )
                    )
                ...
                if intermediate_analyser:
                    ...
                    if plotting_interval is None:
                        ...
                        plot_intermediate_results = epoch % plotting_interval == 0

                    if plot_intermediate_results:
                        if "mixture" in self.latent_distribution_name:
                            n_clusters = self.n_clusters
                            n_latent = self.latent_size
                            if "full-covariance" not in (self.latent_distribution_name):
                                ...
                                for k in range(n_clusters):
                                    p_z_covariances[k] = numpy.diag(p_z_variances[k])
                                    q_z_covariances[k] = numpy.diag(q_z_variances[k])
                            centroids = {
                                "prior": {
                                    "probabilities": p_y_probabilities,
                                    "means": numpy.stack(p_z_means),
                                    "covariance_matrices": (p_z_covariances),
                                },
                                "posterior": {
                                    "probabilities": q_y_probabilities,
                                    "means": q_z_means,
                                    "covariance_matrices": (q_z_covariances),
                                },
                            }
                        else:
                            centroids = None
            ...
            if temporary_log_directory:
                ...
                copying_duration = time() - copying_time_start
                print(
                    "Log directory moved ({}).".format(
                        format_duration(copying_duration)
                    )
                )

                print()

            metadata_log["training duration"] = format_duration(training_duration)
            metadata_log["last epoch duration"] = format_duration(epoch_duration)

            metadata_log_filename = "metadata_log"
            epochs_trained = metadata_log.get("epochs trained")
            if epochs_trained:
                metadata_log_filename += "-" + "-".join(map(str, epochs_trained))
            metadata_log_path = os.path.join(
                self.log_directory(run_id=run_id), metadata_log_filename + ".log"
            )
    ...
    def sample(
        ...
        if self.use_count_sum_as_parameter:
            raise NotImplementedError(
                "Sampling with count sum as reconstruction distribution " "parameter."
            )
        ...
        with tf.Session(graph=self.graph) as session:
            ...
            print("Sampling {} examples from {}.".format(sample_size, model_string))
            sampling_time_start = time()

            y_samples = numpy.empty(
                shape=(sample_size, self.n_clusters), dtype=numpy.int32
            )
            z_mean_samples = numpy.empty(
                shape=(sample_size, self.latent_size), dtype=numpy.float32
            )
            x_mean = numpy.empty(
                shape=(sample_size, self.feature_size), dtype=numpy.float32
            )

            for i in range(0, sample_size, minibatch_size):
                indices = numpy.arange(i, min(i + minibatch_size, sample_size))
                minibatch_sample_size = len(indices)

                (y_samples_i, z_mean_samples_i, z_samples_i) = session.run(
                    [self.p_y_samples, self.p_z_mean_samples, self.p_z_samples],
                ...
                x_mean_i = session.run(self.p_x_mean, feed_dict=feed_dict_batch)
                x_mean[indices] = x_mean_i

            sampling_duration = time() - sampling_time_start

            # Print evaluation
            print("Examples sampled ({}).".format(format_duration(sampling_duration)))
            ...
            sample_z_set = DataSet(
                ...
                example_names=sample_names,
                feature_names=numpy.array(
                    ["z variable {}".format(i + 1) for i in range(self.latent_size)]
                ),
                batch_indices=batch_indices_samples,
                feature_selection=None,
                example_filter=None,
            ...
            sample_y_set = DataSet(
                ...
                example_names=sample_names,
                feature_names=numpy.array(
                    ["y variable {}".format(i + 1) for i in range(self.n_clusters)]
                ),
                batch_indices=batch_indices_samples,
                feature_selection=None,
                example_filter=None,
            ...
            sample_latent_sets = {"z": sample_z_set, "y": sample_y_set}
    ...
    def evaluate(
        ...
        **kwargs,
        ...
        if evaluation_set.label_superset:
            superset_class_names_to_superset_class_ids = numpy.vectorize(
                lambda superset_class_name: evaluation_set.superset_class_name_to_superset_class_id[
                    superset_class_name
                ]
            )
            superset_class_ids_to_superset_class_names = numpy.vectorize(
                lambda superset_class_id: evaluation_set.superset_class_id_to_superset_class_name[
                    superset_class_id
                ]
            )

            evaluation_superset_label_ids = superset_class_names_to_superset_class_ids(
        ...
        with tf.Session(graph=self.graph) as session:
            if log_results:
                eval_summary_writer = tf.summary.FileWriter(eval_summary_directory)
            ...
            if checkpoint:
                ...
                self.saver.restore(session, model_checkpoint_path)
                epoch = int(os.path.split(model_checkpoint_path)[-1].split("-")[-1])
            else:
                raise Exception(
                    "Cannot evaluate {} when it has not been trained.".format(
                        model_string
            ...
            print("Evaluating trained {} on {}.".format(model_string, data_string))
            evaluating_time_start = time()
            ...
            q_y_logits = numpy.zeros(shape=(n_examples_eval, self.n_clusters))

            if "reconstructed" in output_versions:
                p_x_mean_eval = numpy.zeros(
                    shape=(n_examples_eval, n_feature_eval), dtype=numpy.float32
                )
                p_x_stddev_eval = scipy.sparse.lil_matrix(
                    (n_examples_eval, n_feature_eval), dtype=numpy.float32
                )
            ...
            if "latent" in output_versions:
                z_mean_eval = numpy.zeros(
                    shape=(n_examples_eval, self.latent_size), dtype=numpy.float32
                )
                y_mean_eval = numpy.zeros(
                    shape=(n_examples_eval, self.n_clusters), dtype=numpy.float32
                )

            for i in range(0, n_examples_eval, minibatch_size):
                indices = numpy.arange(i, min(i + minibatch_size, n_examples_eval))

                subset_indices = numpy.array(
                    list(evaluation_subset_indices.intersection(indices))
                )
                ...
                feed_dict_batch = {
                    ...
                    self.is_training: False,
                    self.warm_up_weight: 1.0,
                    self.n_iw_samples: self.number_of_importance_samples["evaluation"],
                    self.n_mc_samples: self.number_of_monte_carlo_samples["evaluation"],
                }

                if self.batch_correction:
                    feed_dict_batch[self.batch_indices] = batch_indices_eval[indices]
                ...
                if self.use_count_sum_as_feature:
                    feed_dict_batch[self.count_sum_feature] = count_sum_feature_eval[
                        indices
                    ]
                ...
                if log_results:
                    ...
                    if "full-covariance" in self.latent_distribution_name:
                        q_z_covariances += numpy.array(q_z_covariances_i)
                        p_z_covariances += numpy.array(p_z_covariances_i)
                    kl_divergence_z_neurons += numpy.array(kl_divergence_z_neurons_i)
            ...
            if evaluation_set.has_labels:
                predicted_evaluation_label_ids = map_cluster_ids_to_label_ids(
                    evaluation_label_ids, evaluation_cluster_ids, excluded_class_ids
                )
            ...
            if evaluation_set.label_superset:
                predicted_evaluation_superset_label_ids = map_cluster_ids_to_label_ids(
            ...
            if log_results:
                summary = tf.Summary()
                summary.value.add(
                    tag="losses/lower_bound", simple_value=lower_bound_eval
                )
                summary.value.add(
                    tag="losses/reconstruction_error",
                ...
                summary.value.add(
                    tag="losses/kl_divergence_z", simple_value=kl_divergence_z_eval
                )
                summary.value.add(
                    tag="losses/kl_divergence_y", simple_value=kl_divergence_y_eval
                )
                summary.value.add(tag="accuracy", simple_value=accuracy_eval)
                if accuracy_superset_eval:
                    summary.value.add(
                        tag="superset_accuracy", simple_value=accuracy_superset_eval
                    )
                ...
                for k in range(self.n_clusters):
                    ...
                    for l in range(self.latent_size):
                        ...
                        summary.value.add(
                            tag="prior/cluster_{}/variance/dimension_{}".format(k, l),
                            simple_value=p_z_variances[k][l],
                        )
                        summary.value.add(
                            tag="posterior/cluster_{}/variance/dimension_{}".format(
                                k, l
                            ),
                            simple_value=q_z_variances[k, l],
                        )
            ...
            evaluation_string = "    {} set ({}): ".format(
                evaluation_set.kind.capitalize(), format_duration(evaluating_duration)
            )
            evaluation_metrics = [
            ...
            if "latent" in output_versions:
                ...
                latent_evaluation_sets = {"z": z_evaluation_set, "y": y_evaluation_set}
            ...
            def update_predictions(subset):
                subset.update_predictions(
                    ...
                    predicted_superset_labels=(predicted_evaluation_superset_labels),
                )
    ...
    def _setup_model_graph(self):
            ...
            with tf.variable_scope("P"):
                if self.prior_probabilities_method == "custom":
                    ...
                    p_y_logits = tf.Variable(
                        initial_value=tf.zeros(shape=(self.n_clusters)), name="LOGITS"
                    )
        ...
        with tf.variable_scope("Z"):
            ...
            for k in range(self.n_clusters):
                ...
                self.q_z_given_x_y[k], z_mean[k], self.z[k] = (
                    self._build_graph_for_q_z_given_x_y(
                        self.x,
                        y[k],
                        distribution_name=self.latent_distribution["z posterior"],
                        reuse=reuse_weights,
                    )
                )
                # Latent prior distribution
                ...
                self.p_z_variances.append(
                    tf.square(
                        tf.reduce_mean(self.p_z_given_y[k].stddev(), axis=[0, 1, 2])
                    )
                )

                self.q_z_means.append(
                    tf.reduce_mean(self.q_z_given_x_y[k].mean(), axis=[0, 1, 2])
                )
                ...
                if "full-covariance" in self.latent_distribution_name:
                    self.p_z_covariances.append(
                        tf.reduce_mean(self.p_z_given_y[k].covariance(), axis=[0, 1, 2])
                    )
                    self.q_z_covariances.append(
                        tf.reduce_mean(
            ...
            self.p_z_samples = [
                tf.squeeze(self.p_z_given_y[k].sample(sample_shape=(self.sample_size)))
                for k in range(self.n_clusters)
            ]
            self.p_z_mean_samples = tf.add_n(
                [
                    self.p_z_samples[k] * tf.expand_dims(self.p_y_samples[:, k], -1)
                    for k in range(self.n_clusters)
                ]
            )
    ...
    def _build_graph_for_q_z_given_x_y(
        ...
        with tf.variable_scope("Q"):
            ...
            with tf.variable_scope(normalise_string(distribution_name).upper()):
                ...
                for parameter in distribution["parameters"]:
                    ...
                    theta[parameter] = tf.expand_dims(
                        tf.expand_dims(
                            dense_layer(
                                inputs=encoder,
                ...
                z_samples = q_z_given_x_y.sample(self.n_iw_samples * self.n_mc_samples)
                z = tf.cast(
                    tf.reshape(z_samples, shape=[-1, self.latent_size], name="SAMPLES"),
                    dtype=tf.float32,
                )
    ...
    def _build_graph_for_q_y_given_x(
        ...
        with tf.variable_scope(distribution_name.upper()):
            ...
            theta = {}
            for parameter in distribution["parameters"]:
                parameter_activation_function = distribution["parameters"][parameter][
                    "activation function"
                ]
                p_min, p_max = distribution["parameters"][parameter]["support"]
    ...
    def _build_graph_for_p_x_given_z(self, z, reuse=False):
        ...
        if self.batch_correction:
            batch_indices_one_hot = tf.squeeze(
                tf.one_hot(indices=self.batch_indices, depth=self.number_of_batches),
                axis=1,
            )
        ...
        with tf.variable_scope("DISTRIBUTION"):
            ...
            for parameter in self.reconstruction_distribution["parameters"]:
                parameter_activation_function = self.reconstruction_distribution[
                    "parameters"
            ...
            if (
                ...
                p_x_given_z = self.reconstruction_distribution["class"](
                    x_theta, self.replicated_count_sum_parameter
                )
                ...
                p_x_given_z = self.reconstruction_distribution["class"](x_theta)

            if self.k_max:
                x_logits = dense_layer(
                    inputs=decoder,
                    num_outputs=(
                        self.feature_size * self.number_of_reconstruction_classes
                    ),
                    activation_fn=None,
                    is_training=self.is_training,
                    dropout_keep_probability=self.dropout_keep_probability_h,
                ...
                p_x_given_z = Categorised(
                    dist=p_x_given_z, cat=tfp.distributions.Categorical(logits=x_logits)
                )
    ...
    def _setup_loss_function(self):
        ...
        z_reshaped = [
            tf.reshape(
                self.z[k],
                shape=[self.n_iw_samples, self.n_mc_samples, -1, self.latent_size],
            )
            for k in range(self.n_clusters)
        ]
        ...
        for k in range(self.n_clusters):
            ...
            log_q_z_given_x_y = tf.reduce_sum(
                self.q_z_given_x_y[k].log_prob(z_reshaped[k]), axis=-1
            )
            # (R, L, B, L) --> (R, L, B)
            log_p_z_given_y = tf.reduce_sum(
                self.p_z_given_y[k].log_prob(z_reshaped[k]), axis=-1
            )
            # (R, L, B)
            kl_divergence_z[k] = log_q_z_given_x_y - log_p_z_given_y

            # (R, L, B) --> (B)
            kl_divergence_z_mean[k] = (
                tf.reduce_mean(kl_divergence_z[k], axis=(0, 1)) * self.y[:, k]
            )
            ...
            log_p_x_given_z = tf.reshape(
                tf.reduce_sum(p_x_given_z_log_prob, axis=-1),
                shape=[self.n_iw_samples, self.n_mc_samples, -1],
            )
            # (R, L, B) --> (B)
            log_p_x_given_z_mean[k] = (
                tf.reduce_mean(log_p_x_given_z, axis=(0, 1)) * self.y[:, k]
            )

            # (R * L * B, F) --> (R, L, B, F)
            p_x_given_z_mean = tf.reshape(
                self.p_x_given_z[k].mean(),
                shape=[self.n_iw_samples, self.n_mc_samples, -1, self.feature_size],
            )

            # (R, L, B, F) --> (R, B, F) --> (B, F)
            p_x_means[k] = tf.reduce_mean(
                tf.reduce_mean(p_x_given_z_mean, axis=1), axis=0
            ...
            variance_of_p_x_given_z_means[k] = tf.reduce_mean(
                tf.reduce_mean(
                    tf.square(
                        p_x_given_z_mean
                        - tf.reshape(p_x_means[k], shape=[1, 1, -1, self.feature_size])
                    ),
                    axis=1,
                ),
                axis=0,
        ...
        # K * [(B, F)] --> (B, F)
        self.variance_of_p_x_given_z_mean = tf.add_n(variance_of_p_x_given_z_means)
        self.mean_of_p_x_given_z_variance = tf.add_n(mean_of_p_x_given_z_variances)
        self.p_x_stddev = tf.sqrt(
            self.mean_of_p_x_given_z_variance + self.variance_of_p_x_given_z_mean
        )
        self.stddev_of_p_x_given_z_mean = tf.sqrt(self.variance_of_p_x_given_z_mean)
        ...
        self.reconstruction_error = tf.reduce_mean(tf.add_n(log_p_x_given_z_mean))
        ...
        for k in range(self.n_clusters):
            ...
            kl_divergence_z_neurons_mean[k] = tf.reduce_mean(
                kl_divergence_z_neurons[k], axis=(0, 1)
            ) * tf.expand_dims(self.y[:, k], -1)
            # ) * tf.tile(self.y[:, k], [self.latent_size])

        self.kl_divergence_z_neurons = tf.reduce_mean(
            tf.add_n(kl_divergence_z_neurons_mean), axis=0
        )
    ...
    def _setup_optimiser(self):
        ...
        def _optimiser():
            ...
            clipped_gradients = [
                (tf.clip_by_value(gradient, -1.0, 1.0), variable)
