...
class VariationalAutoencoder:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        parameterise_latent_posterior = kwargs.get("parameterise_latent_posterior")
        ...
        if inference_architecture is None:
            inference_architecture = defaults["models"]["inference_architecture"]
        ...
        if generative_architecture is None:
            generative_architecture = defaults["models"]["generative_architecture"]
        ...
        with self.graph.as_default():
            ...
            self.use_deterministic_z = tf.placeholder(
                dtype=tf.bool, shape=[], name="use_deterministic_z"
            )
    ...
    def description(self):
        ...
        if "mixture" in self.latent_distribution_name:
            description_parts.append(
                "latent clusters: {}".format(self.number_of_latent_clusters)
            )
        if self.parameterise_latent_posterior:
            description_parts.append(
        ...
        if self.kl_weight_value != 1:
            description_parts.append("KL weigth: {}".format(self.kl_weight_value))
            ...
            for epoch in range(epoch_start, number_of_epochs):
                ...
                for i in range(0, n_examples_train, minibatch_size):
                    ...
                    feed_dict_batch = {
                        ...
                        self.number_of_iw_samples: self.number_of_importance_samples[
                            "training"
                        ],
                        self.number_of_mc_samples: self.number_of_monte_carlo_samples[
                            "training"
                        ],
                    }
                ...
                learning_curves["training"]["kl_divergence"].append(kl_divergence_train)
                ...
                training_summary.value.add(
                    tag="losses/kl_divergence", simple_value=kl_divergence_train
                )
                ...
                training_summary_writer.add_summary(
                    training_summary, global_step=epoch + 1
                )
                ...
                if validation_set:
                    ...
                    summary.value.add(
                        tag="losses/kl_divergence", simple_value=kl_divergence_valid
                    )
    ...
    def sample(
        ...
        with tf.Session(graph=self.graph) as session:
            ...
            sample_latent_sets = {"z": sample_z_set}
        ...
        with tf.Session(graph=self.graph) as session:
            ...
            if "reconstructed" in output_versions:
                p_x_mean_eval = numpy.empty(
                    shape=(n_examples_eval, n_features_eval), dtype=numpy.float32
                )
                p_x_stddev_eval = scipy.sparse.lil_matrix(
                    (n_examples_eval, n_features_eval), dtype=numpy.float32
                )
            ...
            for i in range(0, n_examples_eval, minibatch_size):
                ...
                if log_results:
                    kl_divergence_neurons += numpy.array(kl_divergence_neurons_i)
                ...
                if "reconstructed" in output_versions:
                    ...
                    if subset_indices.size > 0:
                        ...
                        # E[x].
                        stddev_of_p_x_mean_eval[subset_indices] = stddev_of_p_x_mean_i[
                            subset_indices - i
                        ]
            ...
            if log_results:
                ...
                summary.value.add(
                    tag="losses/kl_divergence", simple_value=kl_divergence_eval
                )
            ...
            print(
                ...
                "ELBO: {:.5g}, ENRE: {:.5g}, KL: {:.5g}.".format(
                    lower_bound_eval, reconstruction_error_eval, kl_divergence_eval
                ),
            )
            ...
            if "latent" in output_versions:
                ...
                latent_evaluation_sets = {"z": z_evaluation_set}
    ...
    def _setup_model_graph(self):
                ...
                for parameter_name in part_distribution["parameters"]:
                    if parameter_name in part_parameters:
                        part_parameters[parameter_name] = tf.expand_dims(
                            tf.expand_dims(
                                tf.constant(
                                    part_parameters[parameter_name], dtype=tf.float32
                                ),
                                axis=0,
                            ),
                            axis=0,
                    ...
                    parameter_activation_function = part_distribution["parameters"][
                        parameter_name
                    ]["activation function"]
                    p_min, p_max = part_distribution["parameters"][parameter_name][
                        "support"
                    ]
                    initial_value = part_distribution["parameters"][parameter_name][
                        "initial value"
                    ]
                    ...
                    def parameter_variable(num_outputs, name):
                        ...
                        if part_name == "posterior":
                            ...
                            variable = tf.expand_dims(
                                tf.Variable(
                                    activation_function(initial_value([num_outputs])),
                                    name=name,
                                ),
                                axis=0,
                            )
        ...
        if self.parameterise_latent_posterior:
            posterior_parameters = self.latent_distribution["posterior"]["parameters"]
            ...
            for parameter in posterior_parameters:
                if isinstance(posterior_parameters[parameter], list):
                    ...
                    posterior_parameters[parameter] += prior_parameters[parameter]

        self.q_z_given_x = DISTRIBUTIONS[self.latent_distribution["posterior"]["name"]][
            "class"
        ...
        self.p_z = DISTRIBUTIONS[self.latent_distribution["prior"]["name"]]["class"](
        ...
        if "mixture" in self.latent_distribution["prior"]["name"]:
            for k in range(self.number_of_latent_clusters):
                self.p_z_probabilities.append(tf.squeeze(self.p_z.cat.probs)[k])
                self.p_z_means.append(tf.squeeze(self.p_z.components[k].mean()))
                self.p_z_variances.append(
                    tf.diag_part(tf.squeeze(self.p_z.components[k].covariance()))
                )
        else:
            self.p_z_probabilities.append(tf.constant(1.0))
        ...
        with tf.variable_scope("X_TILDE"):
            ...
            if (
                ...
                self.p_x_given_z = self.reconstruction_distribution["class"](
                    theta=x_theta, N=replicated_count_sum_parameter
                )
    ...
    def _setup_loss_function(self):
        ...
        t_tiled = tf.tile(
            self.t, multiples=[self.number_of_iw_samples * self.number_of_mc_samples, 1]
        )
        ...
        if "mixture" in self.latent_distribution_name:
            ...
            self.kl_divergence = tf.reduce_mean(kl_divergence, name="kl_divergence")
            ...
            self.kl_divergence_neurons = tf.reduce_mean(
                tf.reshape(kl_divergence, shape=[-1, self.latent_size]), axis=0
            )
            # KL regularisation term: Sum up KL over all latent dimensions:
            # Shape: ()
            self.kl_divergence = tf.reduce_sum(
                self.kl_divergence_neurons, name="kl_divergence"
            )
            tf.add_to_collection("losses", value=self.kl_divergence)
            ...
            self.p_x_mean = tf.reduce_mean(
                tf.reduce_mean(self.p_x_given_z_mean, axis=1), axis=0
            )
            ...
            self.variance_of_p_x_given_z_mean = tf.reduce_mean(
                tf.reduce_mean(
                    tf.square(
                        self.p_x_given_z_mean
                        - tf.reshape(self.p_x_mean, shape=[1, 1, -1, self.feature_size])
                    ),
                    axis=1,
                ),
                axis=0,
            ...
            #              = 1/(R*L) * ^V[E[x|z]]
            # Shape: (R, L, B, D_x)
            self.variance_of_p_x_mean = self.variance_of_p_x_given_z_mean / tf.cast(
                self.number_of_iw_samples * self.number_of_mc_samples, dtype=tf.float32
            )
            ...
            self.p_x_variance = self.variance_of_p_x_given_z_mean + tf.reduce_mean(
                tf.reduce_mean(self.p_x_given_z_variance, axis=1), axis=0
            )
